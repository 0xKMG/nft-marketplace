...
const MenuItems = ({ isMobile, active, setActive }) => {
  const generateLink = (i) => { // where does i come from?
    switch (i) {
      case 0: return '/';
  ...
  return (
    <ul className={`list-none flexCenter flew-row ${isMobile && 'flex-col h-full'}`}>
      {['Explore NFTs', 'Listed NFTs', 'My NFTs'].map((item, i) => (
        ...
        ${active === item
            ? 'dark:text-white text bg-nft-black-1'
...
const Navbar = () => {
  ...
  const [active, setActive] = useState('Listed NFTs');
